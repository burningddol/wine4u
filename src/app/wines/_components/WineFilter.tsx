...
import { FilterValues, INITIAL_FILTER, WineType } from './WineList';
import { cn } from '@/libs/api/utils';

interface Props {
  currentFilter: FilterValues;
  setCurrentFilter: Dispatch<SetStateAction<FilterValues>>;
  onClose: () => void;
  isDesktop: boolean;
}
...
export default function WineFilter({
  ...
  const [tempFilter, setTempFilter] = useState<FilterValues>(currentFilter);
  ...
  //데스크탑이면 바로 적용, 모바일 필터모달이면 버튼클릭 시 적용
  const handleType = (value: WineType) => {
    setTempFilter((prev) => ({ ...prev, type: value }));

    if (isDesktop) {
      setCurrentFilter((prev) => ({ ...prev, type: value }));
    }
  };
  ...
  const handleRating = (isChecked: boolean, value: number | undefined) => {
    setTempFilter((prev) => ({
      ...prev,
      rating: isChecked ? undefined : value,
    }));
